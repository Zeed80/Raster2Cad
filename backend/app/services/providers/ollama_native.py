from __future__ import annotations

import base64
import json
from pathlib import Path
from typing import Any

import httpx

from app.schemas.models import CapabilityProfile, EngineeringSceneGraph, ModelCheckResult, ModelDescriptor, ModelRuntimeOptions, ProviderType
from app.services.scene_graph_contract import copy_system_prompt, full_scene_graph_schema
from app.services.scene_graph_normalizer import normalize_scene_graph_payload
from app.services.providers.base import BaseModelProvider
from app.services.runtime_profile import recommend_runtime_hints

TINY_PNG_BASE64 = (
    "iVBORw0KGgoAAAANSUhEUgAAAIAAAACACAIAAABMXPacAAACeElEQVR4nO3csWrqUByAcb3t4GZn"
    "H6EOJbpYMup7dPGJhEKhiHTpM1jqqg6CY3yALG66iOLyv0PA+do290P7/aYkEI7k45wg4qlGREWc"
    "P+DYMgDPADADwAwAMwDMADADwAwAMwDMADADwAwAMwDMALDbc2+oVqvlfJLrcdZPLM6AS5sBBX9H"
    "+6nlwRkAMwDMADADwAwAMwDMADADwAwAMwDMADADwAwAMwDMADADwAwAMwDMADADwAwAMwDMADAD"
    "wAwAMwDMADADwAwAMwDMADADwAwAMwDMADADwAxwpQFGo1G73X58fGy3229vb8XFu7u74iDP8yRJ"
    "1ut1SaNfkjjTv9w1Ho/TNN1sNhGx2WzSNP38/IyIer0eEfv9Pk3T+XweV+cLj7SUAN1udzabnU6n"
    "02mv1zsFeHp6en19jWtUOT9AKUvQarVKkuR02mq1siwrjgeDQa1W6/f7ZYx7if7HSzgiir+QH4/H"
    "5+dnl/7SA9zf3y+Xy9PpcrlsNpuVSuXm5maxWOx2u5eXlzLGvUhlLHMfHx9pmm6329NLeDKZnN4B"
    "eZ43Go0sy+LqfOGRlhIgIobDYZIknU6n1WqNRqPiYhEgIt7f3x8eHg6HQ8RvD1A9d9+TYjV3t5Sf"
    "ejh+E4YZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFg"
    "BoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAGYAmAFgBoAZAHb7nX1Z9H3O"
    "ANjZGzbpZzkDYAaAGQBmAJgBYAaAGQBmAJgBYAaAGQBmAJgBYAaosP4CpBzX7Kkv4AcAAAAASUVO"
    "RK5CYII="
)


class OllamaNativeProvider(BaseModelProvider):
    def __init__(self, *, base_url: str, timeout_s: float, num_ctx: int, num_predict: int) -> None:
        self.base_url = base_url.rstrip("/")
        self.timeout_s = timeout_s
        self.num_ctx = num_ctx
        self.num_predict = num_predict

    async def list_models(self) -> list[ModelDescriptor]:
        try:
            async with httpx.AsyncClient(timeout=self.timeout_s) as client:
                response = await client.get(f"{self.base_url}/api/tags")
                response.raise_for_status()
        except Exception:
            return []

        payload = response.json()
        models: list[ModelDescriptor] = []
        for item in payload.get("models", []):
            model_id = item.get("name") or item.get("model") or ""
            details = item.get("details", {})
            models.append(
                ModelDescriptor(
                    id=model_id,
                    display_name=model_id,
                    provider=ProviderType.OLLAMA,
                    capabilities=self._capabilities_for(model_id, details),
                    recommended=model_id.lower() == "qwen3.5:35b",
                    summary=self._build_summary(model_id, details),
                    details=details,
                    runtime_hints=recommend_runtime_hints(provider=ProviderType.OLLAMA, model_id=model_id, details=details),
                )
            )
        return models

    async def check_model(
        self,
        *,
        model_id: str,
        require_vision: bool,
        runtime_options: ModelRuntimeOptions | None = None,
    ) -> ModelCheckResult:
        result = ModelCheckResult(provider=ProviderType.OLLAMA, model_id=model_id)
        try:
            async with httpx.AsyncClient(timeout=self.timeout_s) as client:
                tags_response = await client.get(f"{self.base_url}/api/tags")
                tags_response.raise_for_status()
                result.reachable = True
                models_payload = tags_response.json().get("models", [])
                descriptor = next(
                    (
                        item
                        for item in models_payload
                        if (item.get("name") or item.get("model") or "").lower() == model_id.lower()
                    ),
                    None,
                )
                if descriptor is None:
                    result.error = f"Model {model_id} is not present in Ollama tags."
                    return result

                result.available = True
                capabilities = self._capabilities_for(model_id, descriptor.get("details", {}))
                result.vision_capable = capabilities.vision
                result.details = descriptor.get("details", {})
                num_ctx = min(self._num_ctx(runtime_options), 4096)

                text_payload = {
                    "model": model_id,
                    "stream": False,
                    "think": False,
                    "options": {
                        "num_ctx": num_ctx,
                        "num_predict": 32,
                    },
                    "messages": [{"role": "user", "content": self._with_prompt_controls(model_id, "Reply with exactly ok.")}],
                }
                self._apply_keep_alive(text_payload, runtime_options)
                text_response = await client.post(f"{self.base_url}/api/chat", json=text_payload)
                if text_response.is_success:
                    result.can_text = True
                else:
                    result.error = self._response_error(text_response)
                    return result

                if require_vision:
                    if not result.vision_capable:
                        result.error = f"Model {model_id} is not marked as vision-capable."
                        return result
                    vision_payload = {
                        "model": model_id,
                        "stream": False,
                        "think": False,
                        "options": {
                            "num_ctx": num_ctx,
                            "num_predict": 32,
                        },
                        "messages": [
                            {
                                "role": "user",
                                "content": self._with_prompt_controls(model_id, "Look at the image and reply with exactly ok."),
                                "images": [TINY_PNG_BASE64],
                            }
                        ],
                    }
                    self._apply_keep_alive(vision_payload, runtime_options)
                    vision_response = await client.post(f"{self.base_url}/api/chat", json=vision_payload)
                    if vision_response.is_success:
                        result.can_vision = True
                    else:
                        result.error = self._response_error(vision_response)
                        return result

                result.ok = result.can_text and (result.can_vision if require_vision else True)
                return result
        except Exception as exc:
            result.error = str(exc)
            return result

    async def parse_drawing(
        self,
        *,
        model_id: str,
        source_path: Path,
        prompt: str,
        system_prompt: str | None = None,
        response_schema: dict[str, Any] | None = None,
        temperature: float = 0.0,
        runtime_options: ModelRuntimeOptions | None = None,
    ) -> EngineeringSceneGraph | None:
        image_payload = base64.b64encode(source_path.read_bytes()).decode("utf-8")
        payload = {
            "model": model_id,
            "stream": False,
            "think": False,
            "format": response_schema or "json",
            "options": {
                "temperature": temperature,
                "top_p": 0.1,
                "num_ctx": self._num_ctx(runtime_options),
                "num_predict": self._num_predict(runtime_options),
            },
            "messages": [
                {
                    "role": "system",
                    "content": self._with_prompt_controls(model_id, system_prompt or copy_system_prompt()),
                },
                {
                    "role": "user",
                    "content": prompt,
                    "images": [image_payload],
                },
            ],
        }
        self._apply_keep_alive(payload, runtime_options)
        response_text = await self._chat(payload)
        if not response_text:
            return None
        try:
            return normalize_scene_graph_payload(self._extract_json(response_text))
        except Exception:
            return None

    async def critique_drawing(
        self,
        *,
        model_id: str,
        prompt: str,
        runtime_options: ModelRuntimeOptions | None = None,
    ) -> list[str]:
        payload = {
            "model": model_id,
            "stream": False,
            "think": False,
            "options": {
                "temperature": 0.0,
                "top_p": 0.1,
                "num_ctx": min(self._num_ctx(runtime_options), 8192),
                "num_predict": min(self._num_predict(runtime_options), 512),
            },
            "messages": [
                {"role": "system", "content": "Return concise critique findings as short plain-text lines."},
                {"role": "user", "content": self._with_prompt_controls(model_id, prompt)},
            ],
        }
        self._apply_keep_alive(payload, runtime_options)
        response_text = await self._chat(payload)
        if not response_text:
            return []
        return [line.strip("- ").strip() for line in response_text.splitlines() if line.strip()]

    async def patch_scene_graph(
        self,
        *,
        model_id: str,
        scene_graph: EngineeringSceneGraph,
        instruction: str,
        runtime_options: ModelRuntimeOptions | None = None,
    ) -> EngineeringSceneGraph | None:
        payload = {
            "model": model_id,
            "stream": False,
            "think": False,
            "format": full_scene_graph_schema(),
            "options": {
                "temperature": 0.0,
                "top_p": 0.1,
                "num_ctx": self._num_ctx(runtime_options),
                "num_predict": self._num_predict(runtime_options),
            },
            "messages": [
                {
                    "role": "system",
                    "content": (
                        self._with_prompt_controls(model_id, f"{copy_system_prompt()} ")
                        + (
                        "You update engineering scene graphs after user CAD editing instructions. "
                        "Preserve every unchanged entity. Never drop objects, texts, or dimensions unless the user explicitly asks to delete them. "
                        "Return only JSON."
                        )
                    ),
                },
                {
                    "role": "user",
                    "content": (
                        f"Current counts: objects={len(scene_graph.objects)}, texts={len(scene_graph.texts)}, dimensions={len(scene_graph.dimensions)}.\n"
                        "Return the full updated scene graph with the same top-level keys as the input JSON.\n\n"
                        "Current scene graph JSON:\n"
                        f"{scene_graph.model_dump_json(indent=2)}\n\n"
                        f"User instruction:\n{instruction}"
                    ),
                },
            ],
        }
        self._apply_keep_alive(payload, runtime_options)
        response_text = await self._chat(payload)
        if not response_text:
            return None
        try:
            patched = normalize_scene_graph_payload(self._extract_json(response_text))
        except Exception:
            return None
        patched.notes.append(f"Patched by model {model_id} from chat instruction.")
        return patched

    async def _chat(self, payload: dict[str, Any]) -> str | None:
        try:
            async with httpx.AsyncClient(timeout=self.timeout_s) as client:
                response = await client.post(f"{self.base_url}/api/chat", json=payload)
                response.raise_for_status()
        except Exception:
            return None

        body = response.json()
        message = body.get("message", {})
        content = message.get("content")
        if isinstance(content, str) and content.strip():
            return content
        thinking = message.get("thinking")
        if isinstance(thinking, str) and thinking.strip():
            return thinking
        return None

    def _with_prompt_controls(self, model_id: str, prompt: str) -> str:
        if "qwen3" in model_id.lower() and "/no_think" not in prompt:
            return f"/no_think\n{prompt}"
        return prompt

    def _capabilities_for(self, model_id: str, details: dict[str, Any]) -> CapabilityProfile:
        lowered = model_id.lower()
        families = [item.lower() for item in details.get("families", []) if isinstance(item, str)]
        family = str(details.get("family", "")).lower()
        vision = (
            "qwen3.5" in lowered
            or "vision" in lowered
            or "vl" in lowered
            or "omni" in lowered
            or "qwen35vl" in families
            or "qwen3vl" in families
            or "qwen35vl" == family
            or "qwen3vl" == family
        )
        role_fit = ["critic", "patcher"]
        if vision:
            role_fit = ["parser", "critic", "patcher", "iso-generator"]
        return CapabilityProfile(
            vision=vision,
            reasoning=True,
            tool_calling=False,
            structured_json=True,
            provider=ProviderType.OLLAMA,
            role_fit=role_fit,
        )

    def _build_summary(self, model_id: str, details: dict[str, Any]) -> str:
        size = details.get("parameter_size")
        quant = details.get("quantization_level")
        family = details.get("family")
        chunks = [chunk for chunk in [family, size, quant] if chunk]
        return f"{model_id} ({', '.join(chunks)})" if chunks else model_id

    def _extract_json(self, response_text: str) -> dict[str, Any]:
        text = response_text.strip()
        if text.startswith("```"):
            text = text.strip("`")
            if "\n" in text:
                text = text.split("\n", 1)[1]
            if text.endswith("```"):
                text = text[:-3]
        try:
            return json.loads(text)
        except json.JSONDecodeError:
            start = text.find("{")
            end = text.rfind("}")
            if start >= 0 and end > start:
                return json.loads(text[start : end + 1])
            raise

    def _response_error(self, response: httpx.Response) -> str:
        try:
            payload = response.json()
            if isinstance(payload, dict) and payload.get("error"):
                return str(payload["error"])
        except Exception:
            pass
        return response.text.strip() or f"HTTP {response.status_code}"

    def _num_ctx(self, runtime_options: ModelRuntimeOptions | None) -> int:
        return runtime_options.num_ctx if runtime_options and runtime_options.num_ctx else self.num_ctx

    def _num_predict(self, runtime_options: ModelRuntimeOptions | None) -> int:
        return runtime_options.num_predict if runtime_options and runtime_options.num_predict else self.num_predict

    def _apply_keep_alive(self, payload: dict[str, Any], runtime_options: ModelRuntimeOptions | None) -> None:
        if runtime_options and runtime_options.keep_alive:
            payload["keep_alive"] = runtime_options.keep_alive
